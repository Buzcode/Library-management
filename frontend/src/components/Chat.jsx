...
const Chat = () => {
    ...
    }, [currentUser.Student_id]); // Dependency: re-fetch only if user changes
    ...
    const fetchConversationMessages = useCallback(async (otherUser) => {
        ...
        } catch (error) { console.error("Failed to fetch conversation messages:", error); } 
    ...
    // --- EFFECTS (Now with correct dependencies) ---
    useEffect(() => {
        if (isOpen) {
            fetchUserAndConversationList();
        }
    }, [isOpen, fetchUserAndConversationList]);
    ...
    const renderHeader = () => {
        if (view === 'conversation' && selectedUser) {
            return ( <> <button onClick={handleBackToUsers} className="back-btn"><BsArrowLeft size={20} /></button> <h3>{selectedUser.name}</h3> <button onClick={() => setIsOpen(false)} className="close-chat-btn"><BsX size={24} /></button> </> );
        }
        return ( <> <h3>Messages</h3> <button onClick={() => setIsOpen(false)} className="close-chat-btn"><BsX size={24} /></button> </> );
    };
                        ...
                        {filteredList.length > 0 ? filteredList.map((user) => (
                            <li key={user.id} onClick={() => handleSelectUser(user)}>
                                <div className="user-info">
                                    ...
                                    {user.last_message_time && <span className="last-message-time">{formatTime(user.last_message_time)}</span>}
                                </div>
                                <div className="last-message">
                                    {user.last_message ? user.last_message : `Click to start a conversation`}
                                </div>
                            </li>
                ...
                <>
                    <div className="messages-body" ref={chatBodyRef}>
                        {messages.length > 0 ? messages.map(msg => (
                            <div key={msg.id} className={`message-bubble-container ${msg.sender_id === currentUser.Student_id ? 'sent' : 'received'}`}>
    ...
    return (
        <div className="chat-container">
            {isOpen && ( <div className="chat-window"> <div className="chat-header"> {renderHeader()} </div> {renderBody()} </div> )}
            <button className="chat-bubble" onClick={() => setIsOpen(!isOpen)}> <BsChatDotsFill size={28} /> </button>
        </div>
    );
};
